"""Dynamic, DB-driven RBAC registry client for the execution plane.

The authoritative authorization policy (permissions, roles, role->permission
grants, and the agent/MCP capability catalog with its required permissions, risk
and enabled flags) lives in PostgreSQL and is served by the Node control plane at
``GET /internal/rbac/registry`` (audience-restricted service token, azp pinned).
This module fetches that policy, caches it by its monotonic ``revision`` (revalidated
cheaply with ``If-None-Match``), and publishes it as the process-wide active
registry the Layer B gate reads.

Fail closed: when no registry has been loaded (startup or outage) every lookup
returns "deny" (unknown capability / no granted permission), so a hop is never
authorized against stale-or-absent policy.
"""
...
from ..tokens import ServiceTokenClient, ServiceTokenError
